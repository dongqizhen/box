...
Page({
      ...
      wx.request({
        ...
        success(res) {
          ...
          if (res.statusCode == '200') {
            let content = res.data.data.get_area_info.name + res.data.data.get_area_info.area_name + res.data.data.unit + res.data.data.town
            console.log(content)
  ...
  sharestate: function (box_id) {
    ...
    wx.request({
      ...
      success(res) {
        ...
        if (res.statusCode == '200') {
          if (res.data.data.aliveNum == null || res.data.data.aliveNum == 0 || res.data.data.aliveNum == '') {
            if (res.data.data.allNum == null || res.data.data.allNum == 0 || res.data.data.allNum == '') {
              that.setData({
                sharebox: {
                  aliveNum: 0,
                  allNum: 0
    ...
    wx.scanCode({
      success: (res) => {
        ...
        if (str >= 0) {
          ...
          if (that.data.addressList != '' && that.data.addressList != [] && that.data.addressList != null) {
          ...
          if (res.data == null || res.data == '' || res.data == []) {
            ...
            for (let i = 0; i < res.data.length; i++) {
              ...
              if (saoid == res.data[i].get_cell_info.box_id) {
                let content = res.data[i].get_cell_info.get_area_info.name + res.data[i].get_cell_info.get_area_info.area_name + res.data[i].get_cell_info.get_box_info.town + res.data[i].get_cell_info.get_box_info.unit + res.data[i].get_cell_info.roomNum
                console.log(content) //将柜口内容获取到
                wx.removeStorageSync('boxid')
                wx.setStorageSync('boxid', res.data[i].box_cell_id)
                wx.removeStorageSync('initpswd')
                wx.setStorageSync('initpswd', res.data[i].get_cell_info.initPswd)
                wx.removeStorageSync('iotid')
                wx.setStorageSync('iotid', res.data[i].get_cell_info.IotId)
                wx.removeStorageSync('selectName')
                wx.setStorageSync('selectName', content)
                wx.removeStorageSync('ids')
                wx.setStorageSync('ids', res.data[i].get_cell_info.box_id)
                wx.removeStorageSync('cellnum')
                wx.setStorageSync('cellnum', res.data[i].get_cell_info.cellNum)
                that.setData({
  ...
  LatticeInfo: function () {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.statusCode == '200') {
          if (res.data == null || res.data == '' || res.data == []) {
            ...
            for (var i = 0; i < res.data.length; i++) {
              var address1 = res.data[i].get_cell_info.get_area_info.name;
              var address2 = res.data[i].get_cell_info.get_area_info.area_name;
              var address3 = res.data[i].get_cell_info.get_box_info.town;
              var address4 = res.data[i].get_cell_info.get_box_info.unit;

              var formdata = {
                'address': address1 + address2 + address3 + address4 + res.data[i].get_cell_info.roomNum,
            ...
            if (!wx.getStorageSync('initpswd')) {
              console.log('见来了')
              wx.removeStorageSync("boxid");
              wx.setStorageSync("boxid", addressList[0].box_cell_id)
              wx.removeStorageSync("iotid");
              wx.setStorageSync("iotid", addressList[0].IotId)
              wx.removeStorageSync("cellnum");
              wx.setStorageSync("cellnum", addressList[0].cellNum)
              wx.removeStorageSync('selectName')
              wx.setStorageSync('selectName', addressList[0].address)
              wx.removeStorageSync('initpswd')
              wx.setStorageSync('initpswd', addressList[0].initPswd)
              wx.removeStorageSync('ids')
        ...
        if (res.statusCode == '500' || res.data.message == "Unauthenticated.") {
          console.log('500')
          wx.getSetting({
            complete: (res) => {
  ...
  orderNow(){
    const that = this
    wx.request({
      url: app.globalData.publicAdress + 'api/order/' + this.data.good.orderId,
      method: 'GET',
      header: {
        'content-type': 'application/json', // 默认值
        'Accept': 'application/vnd.cowsms.v2+json',
  ...
  //支付
  pay(res){
    const {appid,timeStamp,package:packages,nonceStr,sign:paySign,signType} = res
    //调起支付
    wx.requestPayment({
      appid,
      timeStamp,
  ...
  sharebox: function (e) {
    ...
    wx.navigateTo({
      url: '/pages/share/share?ids=' + JSON.stringify({
        itcp: this.data.itcp,
          ...
          if (res.data.length != 0) {
            for (let i = 0; i < res.data.length; i++) {
              if (res.data[i].save_stop == null || res.data[i].save_stop == '') {
                ...
                for (let z = 0; z < waybilllist[i].get_use_user.length; z++) {
                  // console.log(waybilllist[i].get_use_user[z])
                  let newphone = waybilllist[i].get_use_user[z].phone.substring(0, 3) + '****' + waybilllist[i].get_use_user[z].phone.substring(7)
                  // console.log(newphone)
                  waybilllist[i].get_use_user[z].newphone = newphone
                }
                // console.log(waybilllist)
    ...
    wx.request({
      ...
      success(res) {
        ...
        if (res.statusCode == 200) {
          // console.log('查看预约与权限')
          console.log(res)
          if (res.data.my_shareBox == 0 && res.data.my_subscribe == 0) {
            that.setData({
              is_mysharebox: false,
              myShareBox: {
